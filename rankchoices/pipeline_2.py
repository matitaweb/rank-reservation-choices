    ...
    for r in kmeans_test_ds.collect():
        ...
        for ar in arguments_col_y:
            ...
            if ar in cluster_freq_dict[c]:
                cfd=cluster_freq_dict[c][ar]
                if(cfd.count() > 0):
                    last_position = cfd.select(last(cfd.POS).alias('last')).collect()[0]['last']
                    
                    # extract rank
                    rankRows = cfd.where(col("IDX").isin({val})).collect()
                    if(len(rankRows) > 0):
                        position = rankRows[0]['POS']
                    else:
                ...
                accuracyDict[ar+"_VAL"]=val
                accuracyDict[ar+"_POS"]=position
                accuracyDict[ar+"_LAST_POS"]=last_position
...
def apply_pca(k_pca, inputCol, outputCol):
    pass
...
def start():
    ...
    # INPUT DATA
    base_filename                   = "data/light_r10.000"
    ...
    #dfi = onehotencoding.add_stringindexer(arguments_col_string, df)
    ...
    # STRING INDEXER
    dfi = add_stringindexer(arguments_col_string, df)
    
    # ONE HOT ENCODING
    df_ohe = get_onehotencoding(arguments_col, df)
    ...
    tot_rows = kmeans_train_ds.count() + kmeans_train_ds.count()
    split=(kmeans_train_ds.count()*100/tot_rows, kmeans_train_ds.count()*100/tot_rows)
    split_col=(kmeans_train_ds.count(), kmeans_train_ds.count())
    
    report_filename = base_filename +".report.txt"
    if os.path.exists(report_filename): os.remove(report_filename)
    write_report(report_filename, tot_col, k_means_num, arguments_col_y, accuracyDictList, accuracyMeanList, time_duration_kmean, time_duration_test, k_pca=k_pca, k_pca_perc=k_pca_perc, split=split, split_col=split_col)
